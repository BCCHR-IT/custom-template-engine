...
class Template
{
    ...
    */
    ...
    private function isSemanticallyEmptyValue($v): bool
    {
        ...
        // Checkbox/matrix arrays: any real entry or non-empty allValues makes it meaningful
        $hasReal = false;
        foreach ($v as $k => $val) {
            if ($k === 'allValues') continue;
            if ($val !== '' && $val !== null) { $hasReal = true; break; }
        }
        $allValues = isset($v['allValues']) ? trim((string)$v['allValues']) : '';
        return !$hasReal && $allValues === '';
    }
            ...
            foreach($json as $index => $event_data)
            {
                ...
                if (!empty($event_data["redcap_repeat_instance"]))
                {
                        ...
                        && !in_array($event_data["redcap_repeat_instrument"] . '|' . $event, $repeatable_instruments_parsed))
                    {
                        ...
                        if (!empty($repeatable_instrument_instances)) {
