...
- Images uploaded through CTE are stored in project-specific subdirectories under the configured Images Folder: `<configured_image_directory>/pid_<pid>/`
- Prevents cross-project image access while maintaining backward compatibility with existing images stored in the root of the configured Images Folder

### Record Image Rendering
- CTE supports rendering images stored in REDCap records:
  - signature fields
  - file upload fields containing images
- Images are retrieved using REDCap's file method and rendered inline in templates
- Files are not copied into the CTE image directory
...
* v4.2.1
  ...
  * fix: restore image rendering in PDFs
  * fix: resolve corrupted ZIP downloads
  * fix: remove debug output affecting generated files
  * chore: finalize dependency updates
    * smarty/smarty (v5.8.0)
    * sabberworm/php-css-parser (v9.3.0)
    * dompdf/dompdf (v3.1.5)
    * dompdf/php-svg-lib (v1.0.2)
* v4.2.0
